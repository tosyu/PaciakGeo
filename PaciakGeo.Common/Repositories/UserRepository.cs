...
namespace PaciakGeo.Common.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        private static class Sql
        {
            ...
            public const string GetUser = @"select uid as uid, name, avatar_url, profile_url, tracking_enabled, last_updated_paciak_location
            ...
            public const string Update = @"update users
                set name = @Name, avatar_url = @AvatarUrl, location = @Location, last_updated_location = @LastUpdatedLocation, location_latitude = @LocationLatitude, location_longitude = @LocationLongitude
                where uid = @Uid";

            public const string DeleteByUid = @"delete * from users where uid = @Uid";
            
            public static string GetUsersForLocationUpdate = @"select * from users
                where tracking_enabled = false AND date_cmp(current_date, last_updated_location) >= 1";
            
            public static string GetUsersForLocationUpdateLimited = @"select * from Users
                where tracking_enabled = false AND date_cmp(current_date, last_updated_location) >= 1
                limit @Limit";
        }
    }
